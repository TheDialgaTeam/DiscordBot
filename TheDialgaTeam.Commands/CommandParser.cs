...
namespace TheDialgaTeam.Commands
{
    internal static class CommandParser
    {
        ...
        public static async Task<ParseResult> ParseArgsAsync(CommandInfo command, string commandString, bool ignoreExtraArgs, IServiceProvider services, string input, int startPos, IReadOnlyDictionary<char, char> aliasMap)
        {
            ...
            for (int curPos = startPos; curPos <= endPos; curPos++)
            {
                ...
                if (argString != null)
                {
                    ...
                    var typeReaderResult = await curParam.ParseAsync(commandString, argString, services).ConfigureAwait(false);
            ...
            if (curParam != null && curParam.IsRemainder)
            {
                var typeReaderResult = await curParam.ParseAsync(commandString, argBuilder.ToString(), services).ConfigureAwait(false);
