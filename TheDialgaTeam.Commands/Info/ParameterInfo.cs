...
namespace TheDialgaTeam.Commands.Info
{
    ...
    public class ParameterInfo
    {
        ...
        public async Task<PreconditionResult> CheckPreconditionsAsync(string commandString, object arg, IServiceProvider services = null)
        {
            services = services ?? EmptyServiceProvider.Instance;

            foreach (var precondition in Preconditions)
            {
                var result = await precondition.CheckPermissionsAsync(commandString, this, arg, services).ConfigureAwait(false);
                if (!result.IsSuccess)
                    return result;
            }
