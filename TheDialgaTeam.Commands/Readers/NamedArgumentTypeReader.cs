...
using TheDialgaTeam.Commands.Utilities;
            ...
            async Task<object> ReadArgumentAsync(PropertyInfo prop, string arg)
            {
                ...
                if (reader != null)
                {
                    if (isCollection)
                    {
                        ...
                        var task = (Task<IEnumerable>)method.Invoke(null, new object[] { reader, commandString, arg.Split(','), services });
                        return await task.ConfigureAwait(false);
                    }
                    else
                        return await ReadSingle(reader, commandString, arg, services).ConfigureAwait(false);
                }
                return null;
            }
        }

        private static async Task<object> ReadSingle(TypeReader reader, string commandString, string arg, IServiceProvider services)
        {
            var readResult = await reader.ReadAsync(commandString, arg, services).ConfigureAwait(false);
